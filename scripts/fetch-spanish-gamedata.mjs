#!/usr/bin/env node
/**
 * Fetch Spanish names AND descriptions for moves, abilities, and items from PokéAPI.
 *
 * Usage: node scripts/fetch-spanish-gamedata.mjs
 * Outputs:
 *   src/lib/i18n/moves.es.json
 *   src/lib/i18n/abilities.es.json
 *   src/lib/i18n/items.es.json
 *   src/lib/i18n/move-descriptions.es.json
 *   src/lib/i18n/ability-descriptions.es.json
 *   src/lib/i18n/item-descriptions.es.json
 */
...
// ── Extract canonical keys from the dictionaries the app renders ───────
...
function pickFlavor(entries) {
  ...
  // Prefer newest scarlet-violet, then sword-shield, then any latest Spanish entry
...
async function fetchOne(endpoint, slug) {
  ...
  try {
    ...
    // Flavor text: moves/abilities use flavor_text_entries directly.
    // Items use flavor_text_entries with { text, language, version_group }.
    let description = null;
    const flavor = pickFlavor(data.flavor_text_entries);
    if (flavor) {
      description = cleanText(flavor.flavor_text ?? flavor.text);
    }
    // Fallback: effect_entries (abilities/items), usually English only
    if (!description) {
...
async function main() {
  console.log("\nFetching Spanish game data from PokeAPI...\n");
  ...
  console.log("");
  write("moves.es.json", moves.resultNames);
  write("move-descriptions.es.json", moves.resultDescs);
  write("abilities.es.json", abilities.resultNames);
  write("ability-descriptions.es.json", abilities.resultDescs);
  write("items.es.json", items.resultNames);
  write("item-descriptions.es.json", items.resultDescs);
}
