#!/usr/bin/env node
/**
 * Fetch Italian names AND descriptions for moves, abilities, items, and natures from PokéAPI.
 *
 * Usage: node scripts/fetch-italian-gamedata.mjs
 * Outputs:
 *   src/lib/i18n/moves.it.json
 *   src/lib/i18n/abilities.it.json
 *   src/lib/i18n/items.it.json
 *   src/lib/i18n/natures.it.json
 *   src/lib/i18n/move-descriptions.it.json
 *   src/lib/i18n/ability-descriptions.it.json
 *   src/lib/i18n/item-descriptions.it.json
 */
...
async function fetchNatures(names) {
  ...
  for (let i = 0; i < names.length; i += BATCH_SIZE) {
    ...
    const promises = batch.map(async (name) => {
      ...
      const itName = await fetchNature(slug);
      if (itName) {
        result[name] = itName;
      } else {
...
async function main() {
  console.log("\n🇮🇹 Fetching Italian game data from PokéAPI...\n");
  ...
  console.log("");
  write("moves.it.json", moves.resultNames);
  write("move-descriptions.it.json", moves.resultDescs);
  write("abilities.it.json", abilities.resultNames);
  write("ability-descriptions.it.json", abilities.resultDescs);
  write("items.it.json", items.resultNames);
  write("item-descriptions.it.json", items.resultDescs);
  write("natures.it.json", natures.result);
}
